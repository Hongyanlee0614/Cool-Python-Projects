...
from playsound import playsound  # can play a sound file with a single code line
import os

################### Initialized window####################

root = Tk()
root.geometry('350x300')  # width x heigjht
root.resizable(0, 0)  # cannot resize
root.config(bg='ghost white')  # configure is to access window attribute
...
def Text_to_speech():
    ...
    # another parameters including 'lang' and 'slow'
...
def Reset():
    Msg.set("")
    # remove as we press reset so that will not cause PermissionError
